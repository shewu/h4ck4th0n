...
void HolyGameViewController::_initMenus() {
	mainmenu = new menu();
...
void HolyGameViewController::_initSound() {
#ifndef __APPLE__
	ALCdevice* dev;
	ALCcontext* con;
...
HBViewMode HolyGameViewController::didFinishView() {
	return finishedView;
}
...
void HolyGameViewController::_disconnect() {
	if (sc) {
		char q = 0;
		sc->add(&q, 1);
		sc->send();
...
bool HolyGameViewController::quit() {
	_disconnect();
	exit(0);
}

bool HolyGameViewController::leave() {
	_disconnect();
