...
void HolyGameViewController::_initGL() {
...
#ifndef __APPLE__
        CL_GLX_DISPLAY_KHR, (intptr_t)glXGetCurrentDisplay(), CL_GL_CONTEXT_KHR,
        (intptr_t)glXGetCurrentContext(),
#else
        CL_CONTEXT_PROPERTY_USE_CGL_SHAREGROUP_APPLE,
        (cl_context_properties)kCGLShareGroup, NULL, NULL,
#endif
        0};
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, WIDTH, HEIGHT, 0, GL_RGBA,
                 GL_UNSIGNED_BYTE, 0);
    igl = cl::ImageGL(context, CL_MEM_WRITE_ONLY, GL_TEXTURE_2D, 0, texture,
                      NULL);
    bs.push_back(igl);
...
void HolyGameViewController::render() {
    ...
    cl::Buffer obspointsbuf(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                            4 * ti * sizeof(float), obspoints, NULL);
    cl::Buffer obscolorbuf(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
    ...
    cl::Buffer objpointbuf(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                           2 * roundObjects.size() * sizeof(float), objpoint,
                           NULL);
    cl::Buffer objsizebuf(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                          2 * roundObjects.size() * sizeof(float), objsize,
                          NULL);
    cl::Buffer objcolorbuf(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                           4 * roundObjects.size() * sizeof(char), objcolor,
                           NULL);
    ...
    cl::Buffer lightposbuf(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                           3 * world->getLights().size() * sizeof(float),
                           lightpos, NULL);
    cl::Buffer lightcolorbuf(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                             4 * world->getLights().size() * sizeof(char),
                             lightcolor, NULL);
    ...
    cq.enqueueNDRangeKernel(
        renderKern, cl::NullRange,
        cl::NDRange((WIDTH + 15) / 16 * 16, (HEIGHT + 15) / 16 * 16),
        cl::NDRange(16, 16));
