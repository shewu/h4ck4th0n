...
public:
    SpawnComponentLine(Vector2D const &point1, Vector2D const &point2)
...
public:
    SpawnComponentRectangle(Vector2D const &point1, Vector2D const &point2)
        : _point1(point1), _point2(point2) {}
    float getArea() const;
    Vector2D getRandomPoint() const;
...
public:
    SpawnComponentTriangle(Vector2D const &point1, Vector2D const &point2,
                           Vector2D const &point3)
...
public:
    SpawnComponentSlice(Vector2D center, float radius, float theta1,
                        float theta2)
...
public:
    SpawnComponentSector(Vector2D center, float radius, float theta1,
                         float theta2)
        : _center(center), _radius(radius), _theta1(theta1), _theta2(theta2) {}
    float getArea() const;
    Vector2D getRandomPoint() const;
...
public:
    ...
    WallType getWallType() const { return _wallTypeData.wallType; }

    MaterialPtr getMaterial() const { return _wallTypeData.material; }

    float getBouncinessCoefficient() const { return _bouncinessCoefficient; }
...
public:
    RectangularWallDescriptor(WallTypeData wallTypeData_, Vector2D pos1_,
                              Vector2D pos2_, float bouncinessCoefficient_)
        : WallDescriptor(wallTypeData_, bouncinessCoefficient_),
          _pos1(pos1_),
          _pos2(pos2_) {}
    Vector2D getPos1() const { return this->_pos1; }
    Vector2D getPos2() const { return this->_pos2; }
...
public:
    RoundWallDescriptor(WallTypeData wallTypeData_, Vector2D center_,
                        float radius_, float theta1_, float theta2_,
                        float bouncinessCoefficient_)
        : WallDescriptor(wallTypeData_, bouncinessCoefficient_),
          _center(center_),
          _radius(radius_),
          _theta1(theta1_),
    ...
    float getTheta1() const { return this->_theta1; }
    float getTheta2() const { return this->_theta2; }
...
public:
    ...
    /**
     * Returns true if the point resides in the region covered by this floor,
     * and
     * false otherwise. In the case that it returns true, it puts the friction
     * coefficient
     * of the floor at that point in the 'coeff' argument. In the false case,
     * coeff can be
     * anything.
     */
