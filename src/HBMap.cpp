...
public:
    StringTokenizer(string const &s, string delim = " \t") { init(s, delim); }
    ...
    bool hasMoreTokens() const { return toks.size() > 0; }
...
float string2angle(string const &s) {
    ...
    if (angleDegrees <= -360 || angleDegrees > 360) {
        throw ParseException("failure to parse angle: " + s +
                             " should be in (-360, 360]");
    }
...
void HBMap::parseDimensions(string const &s) {
    ...
    if (tl.args.size() != 2) {
        throw ParseException(
            "failure parsing dimension: 2 arguments expected\n");
    }
...
void HBMap::parseSpawn(string const &s) {
    ...
    if (tl.args.size() < 2) {
        throw ParseException(
            "failure parsing spawn: type and index must be specified\n");
    }
    int index = string2int(tl.args[0]);
    string const &type = tl.args[1];
    if (index < 0 || index > kMaxSpawnRegionNumber) {
        throw ParseException(
            "failure parsing spawn: region number out of bounds");
    }
    Vector2D a, b, c;
    if (type == "point") {
        if (tl.args.size() != 4) {
            throw ParseException(
                "failure parsing spawn: failure parsing point");
        }
        a.x = string2float(tl.args[2]);
        a.y = string2float(tl.args[3]);
        spawns[index].addComponent(
            make_shared<SpawnComponentPoint>(SpawnComponentPoint(a)));
    } else if (type == "line") {
        if (tl.args.size() != 6) {
        ...
        if (tl.args.size() != 6) {
            throw ParseException(
                "failure parsing spawn: failure parsing rectangle");
        }
        a.x = string2float(tl.args[2]);
        a.y = string2float(tl.args[3]);
        b.x = string2float(tl.args[4]);
        ...
        if (tl.args.size() != 8) {
            throw ParseException(
                "failure parsing spawn: failure parsing triangle");
        }
        a.x = string2float(tl.args[2]);
        a.y = string2float(tl.args[3]);
        b.x = string2float(tl.args[4]);
        ...
        spawns[index].addComponent(
            make_shared<SpawnComponentTriangle>(a, b, c));
    } else if (type == "slice" || type == "sector") {
        if (tl.args.size() != 7) {
            throw ParseException(
                "failure parsing spawns: failure parsing arc slice");
        }
        a.x = string2float(tl.args[2]);
        a.y = string2float(tl.args[3]);
        ...
        if (type == "arcslice") {
            spawns[index].addComponent(
                make_shared<SpawnComponentSlice>(a, radius, theta1, theta2));
        } else {
            spawns[index].addComponent(
                make_shared<SpawnComponentSector>(a, radius, theta1, theta2));
        }
    } else {
        throw ParseException("failure parsing spawn: urecognized spawn type " +
                             type);
    }
}
...
void HBMap::parseRectangularWall(string const &s) {
    ...
    if (tl.args.size() != 5) {
        throw ParseException(
            "failure parsing rectangular wall: 5 arguments expected\n");
    }
    ...
    if (tl.attributes["bounciness"] != "") {
        ...
        if (bc < 0.0f || bc > 1.0f) {
            throw ParseException(
                "failure parsing rectangular wall: invalid bounciness: " +
                tl.attributes["bounciness"]);
        }
    }

    rectangularWalls.push_back(RectangularWallDescriptor(
        wallTypeData, Vector2D(a, b), Vector2D(c, d), bc));
}
...
void HBMap::parseRoundWall(string const &s) {
    ...
    if (tl.args.size() != 6) {
        throw ParseException(
            "failure parsing round wall: 6 arguments expected\n");
    }
    WallTypeData wallTypeData = parseWallType(tl.args[0]);
    int a = string2int(tl.args[1]);
    int b = string2int(tl.args[2]);
    ...
    if (tl.attributes["bounciness"] != "") {
        ...
        if (bc < 0.0f || bc > 1.0f) {
            throw ParseException(
                "failure parsing round wall: invalid bounciness: " +
                tl.attributes["bounciness"]);
        }
    }

    roundWalls.push_back(
        RoundWallDescriptor(wallTypeData, Vector2D(a, b), c, d, e, bc));
}
