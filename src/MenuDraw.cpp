...
void Menu::drawMenu() {
    ...
    glPushMatrix();
    glLoadIdentity();
    glOrtho((-x1 - WIDTH / float(HEIGHT) / 2.0f + 1 / 2.0f) / (x2 - x1),
            (1.0f - x1 + WIDTH / float(HEIGHT) / 2.0f - 1 / 2.0f) / (x2 - x1),
            (1.0f - y1) / (y2 - y1), -y1 / (y2 - y1), -1.0f, 1.0f);
...
void Menu::draw() {
    if (!is_item_active || menuItems[current_index]->shouldMenuBeDrawn()) {
        ...
        for (int i = 0; i < static_cast<int>(menuItems.size()); i++) {
            menuItems[i]->draw(
                i == current_index, kMenuDrawButtonLeft,
                kMenuDrawButtonTop +
                    i * (kMenuDrawButtonHeight + kMenuDrawButtonSeparation),
                kMenuDrawButtonWidth, kMenuDrawButtonHeight,
                transparent ? kMenuDrawMenuAlpha : 255);
        }
    }

    if (is_item_active) {
        menuItems[current_index]->drawAsActive(transparent ? kMenuDrawMenuAlpha
                                                           : 255);
    }
}
...
static void draw_button(bool selected, string const &text, float x, float y,
                        float width, float height, GLubyte alpha) {
    if (selected) {
    ...
    textquad tq(
        x + kMenuDrawButtonTextPadding, y + (height - kMenuDrawFontSize) * 0.5f,
        0.0f, x + kMenuDrawButtonTextPadding,
        y + (height + kMenuDrawFontSize) * 0.5f, 0.0f, 0.05f, 0.0f, 0.0f);
    draw_str(tq, text);
}
...
void MenuItem::draw(bool selected, float x, float y, float width, float height,
                    unsigned char alpha) {
    draw_button(selected, name_.c_str(), x, y, width, height, alpha);
}

void SubMenuItem::draw(bool selected, float x, float y, float width,
...
void InputMenuItem::drawAsActive(unsigned char alpha) {
    glBegin(GL_QUADS);
    {
        ...
        glVertex3f(kMenuDrawInputLeft + kMenuDrawInputTextfieldLeft +
                       kMenuDrawInputTextfieldWidth,
                   kMenuDrawInputTop + kMenuDrawInputTextfieldTop, 0.0f);
        glVertex3f(kMenuDrawInputLeft + kMenuDrawInputTextfieldLeft +
                       kMenuDrawInputTextfieldWidth,
                   kMenuDrawInputTop + kMenuDrawInputTextfieldTop +
                       kMenuDrawInputTextfieldHeight,
                   0.0f);
        glVertex3f(kMenuDrawInputLeft + kMenuDrawInputTextfieldLeft,
                   kMenuDrawInputTop + kMenuDrawInputTextfieldTop +
                       kMenuDrawInputTextfieldHeight,
    ...
    if ("" != text) {
        textquad tq(
            ...
            kMenuDrawInputTop + kMenuDrawInputTitleTop, 0.0f,
            kMenuDrawInputLeft + kMenuDrawInputTitleLeft,
    ...
    if (displayError && "" != invalidInputError) {
        ...
        textquad tq(
            kMenuDrawInputLeft + kMenuDrawInputTextfieldLeft + 0.02f,
            kMenuDrawInputTop + kMenuDrawInputTextfieldTop +
                kMenuDrawInputTextfieldHeight + 0.02f,
            0.0f, kMenuDrawInputLeft + kMenuDrawInputTextfieldLeft + 0.02f,
            kMenuDrawInputTop + kMenuDrawInputTextfieldTop +
                kMenuDrawInputTextfieldHeight + kMenuDrawFontSize + 0.02f,
            0.0f, 0.05f, 0.0f, 0.0f);
        draw_str(tq, invalidInputError);
    }
}

void ToggleMenuItem::draw(bool selected, float x, float y, float width,
                          float height, unsigned char alpha) {
    draw_button(selected, name_, x, y, width, height, alpha);
    textquad tq(x + width - kMenuDrawToggleDistanceFromBack,
                y + (height - kMenuDrawFontSize) * 0.5f, 0.0f,
                x + width - kMenuDrawToggleDistanceFromBack,
                y + (height + kMenuDrawFontSize) * 0.5f, 0.0f, 0.05f, 0.0f,
                0.0f);
    if (state) {
...
void SliderMenuItem::draw(bool selected, float x, float y, float width,
                          float height, unsigned char alpha) {
    glColor4ub(0, 0, 0, alpha);
    glBegin(GL_LINES);
    {
        glVertex3f(x + slider_left * width, y + slider_line_height * height,
                   0.0f);
        glVertex3f(x + slider_right * width, y + slider_line_height * height,
                   0.0f);
        for (int i = 0; i < static_cast<int>(states.size()); i++) {
            float tickx = x +
                          width *
                              (slider_right * (float)i +
                               slider_left * (float)(states.size() - 1 - i)) /
                              (float)(states.size() - 1);
    ...
    float rectx1 =
        x +
        width * ((slider_right * (float)curstate +
                  slider_left * (float)(states.size() - 1 - curstate)) /
                     (float)(states.size() - 1) -
                 0.5f * slider_slide_width);
    float recty1 =
        y + height * (slider_line_height - 0.5f * slider_slide_height);
    float recty2 =
        y + height * (slider_line_height + 0.5f * slider_slide_height);
    glBegin(GL_QUADS);
    ...
    if (curstate != newcurstate) {
        ...
        rectx1 =
            x +
            width * ((slider_right * (float)newcurstate +
                      slider_left * (float)(states.size() - 1 - newcurstate)) /
                         (float)(states.size() - 1) -
                     0.5f * slider_slide_width);
        glVertex3f(rectx1, recty1, 0.0f);
        glVertex3f(rectx1, recty2, 0.0f);
    ...
    for (int i = 0; i < static_cast<int>(states.size()); i++) {
        float tickx = x +
                      width * (slider_right * (float)i +
                               slider_left * (float)(states.size() - 1 - i)) /
                          (float)(states.size() - 1);
        textquad tq(tickx, y + slider_text_top * height, 0.0f, tickx,
                    y + slider_text_bottom * height, 0.0f,
                    (slider_text_bottom - slider_text_top) * height, 0.0f,
                    0.0f);
        draw_str_center(tq, states[i]);
    }
    textquad tq(
        slider_text_name_left * width, y + slider_text_name_top * height, 0.0f,
        slider_text_name_left * width, y + slider_text_name_bottom * height,
        0.0f, (slider_text_name_bottom - slider_text_name_top) * height, 0.0f,
        0.0f);
    draw_str(tq, name_);
}
