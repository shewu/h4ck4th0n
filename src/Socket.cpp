...
void Socket::closeConnection(SocketConnection *sc) {
    connections.erase(pair<string, int>(
        string((char *)sc->get_addr(), sc->get_addrlen()), sc->get_their_id()));
    delete sc;
}
...
SocketConnection *Socket::connect(sockaddr *addr, socklen_t addrlen) {
    int id = rand();
    SocketConnection *sc =
        new SocketConnection(socket, addr, addrlen, id, SERVER_ID);
...
void Socket::recv_all() {
    ...
    while (true) {
        ...
        ssize_t len =
            recvfrom(socket, buf, MAXPACKET, MSG_DONTWAIT, &addr, &addrlen);
        if (len < 8)
            break;
        int magic;
        memcpy((void *)&magic, (void *)buf, 4);
        magic = ntohl(magic);
        if (magic != MESSAGE_HEADER_INIT)
            continue;
